...
void ds();
void dis(char n1[10], char n2[10]);
...
int main()
{
    ...
    while (restart == 1)
    {
        ...
        printf("Enter '0' for Player VS Player || '1' for player VS computer:\n"); // getting input for play type whether pv or 1v1
        do
        {
            scanf("%d", &pvc);
            if (pvc > 1)
...
void  dis(char n1 [10], char n2[10])
{
    ...
    for (int i = 1; i <= 5; i++) // 5 time statement will blink
        {
            system("cls"); // clear the screen
            msleep(400); // wait for a second
            printf("\t\t\t\t\t\t\t\t ----------------------------------------------------- \n");
            printf("\t\t\t\t\t\t\t\t\t\t\033[1;31m %s \033[0m VS \033[1;34m %s \033[0m \t      \n", n1,n2);  // print this statement on screen.
            printf("\t\t\t\t\t\t\t\t ----------------------------------------------------- \n");
            msleep(400);
